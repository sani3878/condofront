...
def receive():
    if request.method == 'POST':
        ...
        parcel_type  = request.form.get('parcel_type', 'box').strip()
        ...
        pickup_code = 'PKP-' + _s.token_urlsafe(4).upper()[:6]

        cur.execute("""
            INSERT INTO tblparcel
                (property_id, room_id, courier_id, tracking_no,
                 note, parcel_type, pickup_code, received_by, status_id)
...
def pickup(parcel_id):
    ...
    if request.method == 'POST':
        ...
        pickup_method  = request.form.get('pickup_method', 'manual')
        ...
        cur.execute("""
            INSERT INTO tblpickup
                (property_id, room_id, parcel_id, signature_path,
                 pickup_note, handled_by, pickup_method)
...
def qr_pickup(code):
    ...
    parcel = query_one("""
        SELECT p.*, r.room_no, r.building, c.courier_name
        FROM tblparcel p
        JOIN tblroom r ON p.room_id = r.idno
        LEFT JOIN tblcourier c ON p.courier_id = c.idno
        WHERE p.pickup_code = %s
          AND p.property_id = %s
          AND p.status_id = 0
    """, [code.upper(), current_user.property_id])

    if not parcel:
        flash(f'ไม่พบพัสดุสำหรับรหัส {code} หรือรับไปแล้ว', 'warning')
        return redirect(url_for('parcel.list_parcels'))
    ...
    if request.method == 'POST':
        ...
        cur.execute("""
            INSERT INTO tblpickup
                (property_id, room_id, parcel_id,
                 handled_by, pickup_method)
            VALUES (%s, %s, %s, %s, 'qr')
            RETURNING idno
        """, [current_user.property_id,
              parcel['room_id'],
        ...
        cur.execute("""
            ...
            SET status_id  = 1,
                ...
                updated_at = NOW()
            WHERE idno = %s
        """, [pickup_id, current_user.id, parcel['idno']])

        db.commit()
        flash(f'✅ QR Pickup สำเร็จ! ห้อง {parcel["building"] or ""}{parcel["room_no"]}', 'success')
        return redirect(url_for('parcel.list_parcels'))

    return render_template('parcel/qr_pickup.html',
        active_page='list',
        parcel=parcel)
